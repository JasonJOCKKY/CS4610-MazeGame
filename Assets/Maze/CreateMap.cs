...
public class Maze {
  public Floor entranceFloor, exitFloor;
  public Wall entranceWall, exitWall;
}
...
public class CreateMap : MonoBehaviour
{
  ...
  public GameObject test_wallPrefab;
  ...
  public void CreateGround()
  {
    for (int i = 0; i < X; i++)
    {
      for (int j = 0; j < Y; j++)
      {
        Floor item = Instantiate(floorPrefab).GetComponent<Floor>();
  ...
  public void CreateWall2()
  {
    for (int i = 0; i < X + 1; i++)
    {
      for (int j = 0; j < Y; j++)
      {
        Wall item = Instantiate(test_wallPrefab).GetComponent<Wall>();
  ...
  public Maze CreateStep1()
  {
    ...
    for (int i = 0; i < road.Count - 1; i++)
    {
      ...
      {
        // Entrance
        newMaze.entranceFloor = cubes[(int)road[i].x, (int)road[i].y];
        newMaze.entranceWall = cubes[(int)road[i].x, (int)road[i].y].neighborWalls[1];
        
        newMaze.entranceWall.gameObject.SetActive(false);
        newMaze.entranceFloor.neighborWalls[1] = null;
      ...
      {
        // Exit
        newMaze.exitFloor = cubes[(int)road[i + 1].x, (int)road[i + 1].y];
        newMaze.exitWall = cubes[(int)road[i + 1].x, (int)road[i + 1].y].neighborWalls[0];

        newMaze.exitWall.gameObject.SetActive(false);
        newMaze.exitFloor.neighborWalls[0] = null;
